...
async function loadEventDetails() {
    ...
    try {
        const data = await getEventByIdFromDB(eventId);
        ...
        showMessage(`Could not load event details: ${error.message || "please try again."}`);
    }
}
...
async function updateStatus(newStatus) {
    ...
    try {
        const updated = await updateEventInDB(currentEvent.id, { event_status: newStatus });
        ...
        showPortalToast(`Could not update the event: ${error.message || "please try again."}`, "error");
    }
}
...
async function deleteEvent() {
    ...
    try {
        await deleteEventFromDB(currentEvent.id);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("editForm").addEventListener("submit", async event => {
        ...
        try {
            const updated = await updateEventInDB(currentEvent.id, updates);
            currentEvent = updated;
